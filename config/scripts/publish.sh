...
echo "=æ Publishing STOMP WebSocket Client Library..."
...
if ! git diff-index --quiet HEAD --; then
  echo "L Uncommitted changes detected. Please commit all changes before publishing."
  exit 1
fi
...
echo "=Ý Current version: $(node -p "require('./package.json').version")"
...
if [ "$VERSION_BUMP" != "skip" ] && [ "$VERSION_BUMP" != "" ]; then
  if [[ "$VERSION_BUMP" =~ ^(major|minor|patch)$ ]]; then
    ...
    echo "L Invalid version bump type. Use: major, minor, patch, or skip"
    exit 1
  fi
fi
...
# Publish to npm
echo "=ä Publishing to npm..."
npm publish --access public

if [ $? -eq 0 ]; then
  echo "<‰ Successfully published STOMP WebSocket Client Library!"
  echo "=æ Package: @rfice/stomp@$(node -p "require('./package.json').version")"
else
  echo "L Publishing failed"
  exit 1
fi