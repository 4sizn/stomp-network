...
echo "🏗️  Building STOMP WebSocket Client Library..."
...
# Verify build output
if [ -d "dist" ] && [ -f "dist/index.umd.js" ] && [ -f "dist/index.d.ts" ]; then
  ...
  # Verify all formats (ESM and UMD)
  if [ -f "dist/index.umd.js" ] && [ -f "dist/index.es.js" ]; then
    echo "✅ All formats (ESM, UMD) generated successfully"
  fi

  # Additional validation for STOMP library
  echo "🔍 Validating STOMP library build..."
  ...
  if grep -q "StompClient\|useStompConnection\|StompProvider" dist/index.d.ts 2>/dev/null; then
    ...
    echo "⚠️  Warning: Could not verify STOMP exports in type definitions"
  fi
